...
def set_up_data(H):
    ...
    if H.dataset == 'imagenet32':
        ...
        trX, vaX, teX = celeba(H.data_root)
        H.image_size = 64
        H.image_channels = 3
        shift = -115.92961967
        scale = 1. / 69.37404
    ...
    def preprocess_func(x):
        ...
        inp = x[0].cuda(non_blocking=True).float() #inp = 255.*x[0].cuda(non_blocking=True).float()
        out = inp.clone()
        #inp += 15*torch.randn_like(inp, device=inp.device)  # Denoising Criterion
...
def ffhq256(data_root):
    ...
    np.random.seed(5)
    #tr_va_split_indices = np.random.permutation(trX.shape[0])
    train = trX[:-7000]#train = trX[tr_va_split_indices[:-7000]]
    valid = trX[-7000:]#valid = trX[tr_va_split_indices[-7000:]]
    # we did not significantly tune hyperparameters on ffhq-256, and so simply evaluate on the test set
    return train, valid, valid

def celeba(data_root):
    return os.path.join(data_root, 'celeba'), os.path.join(data_root, 'celeba'), os.path.join(data_root, 'celeba')
