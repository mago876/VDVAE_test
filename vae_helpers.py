...
##### Custom function (from ''), used in 'vae.py: DecBlock.forward_eps' #####
@torch.jit.script
def draw_gaussian_eps(mu, logsigma):
    eps = torch.empty_like(mu).normal_(0., 1.)
...
def sample_from_discretized_mix_logistic(l, nr_mix):
    ...
    x0 = const_min(const_max(x[:, :, :, 0], -1.), 1.)  # R channel
    x1 = const_min(const_max(x[:, :, :, 1] + coeffs[:, :, :, 0] * x0, -1.), 1.)  # G channel (conditioned on R chanel)
    x2 = const_min(const_max(x[:, :, :, 2] + coeffs[:, :, :, 1] * x0 + coeffs[:, :, :, 2] * x1, -1.), 1.)  # B channel (conditioned on R and G channels)
    return torch.cat([torch.reshape(x0, xs[:-1] + [1]), torch.reshape(x1, xs[:-1] + [1]), torch.reshape(x2, xs[:-1] + [1])], dim=3)

##### Custom function (from 'sample_from_discretized_mix_logistic'), used in '' #####
def params_from_gaussian(l, nr_mix, gamma=None):
    ls = [s for s in l.shape]
    xs = ls[:-1] + [3]
    # unpack parameters
    logit_probs = l[:, :, :, :nr_mix]
    ...
    sel = torch.reshape(sel, xs[:-1] + [1, nr_mix])
    # select logistic parameters
    mu_D = (l[:, :, :, :, :nr_mix] * sel).sum(dim=4)
    log_scales = const_max((l[:, :, :, :, nr_mix:nr_mix * 2] * sel).sum(dim=4), -7.)
    coeffs = (torch.tanh(l[:, :, :, :, nr_mix * 2:nr_mix * 3]) * sel).sum(dim=4)

    # sample from GAUSSIAN & clip to interval
    # we don't actually round to the nearest 8bit value when sampling
    if gamma is None:
        #gamma = np.pi*torch.exp(log_scales) / np.sqrt(3)
        gamma = 0.01#torch.mean(gamma)
    #print('sigma (sample_from_gaussian):', sigmas)
    ...
    mu_D = torch.cat([torch.reshape(x0, xs[:-1] + [1]), torch.reshape(x1, xs[:-1] + [1]), torch.reshape(x2, xs[:-1] + [1])], dim=3)
    #x = means + torch.randn_like(means, device=means.device) * sigmas
    return mu_D, gamma
...
class DmolNet(nn.Module):
    ...
    ##### Custom function (from 'sample') #####
    def sample_gaussian(self, px_z):
        means, sigma = params_from_gaussian(self.forward(px_z), self.H.num_mixtures)
        means = (means + 1.0) / 2  # Output image with dynamic range [0,1]
        x = means + torch.randn_like(means, device=means.device) * sigma
        x = const_min(const_max(x, 0.0), 1.0)#.astype(np.uint8)
        return x
    ###########################################

    ##### Custom function (from 'sample') #####
    def params_gaussian(self, px_z):
        #im = sample_from_gaussian(self.forward(px_z), self.H.num_mixtures)
        mu_D, gamma = params_from_gaussian(self.forward(px_z), self.H.num_mixtures)
        #print(im.min(), im.max())
        mu_D = (mu_D + 1.0) / 2  # Output image with dynamic range [0,1]
        #xhat = xhat.detach().cpu().numpy()
        mu_D = const_min(const_max(mu_D, 0.0), 1.0)#.astype(np.uint8)
        return mu_D, gamma
    ###########################################